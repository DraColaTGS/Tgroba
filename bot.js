...
var prefix = "="
  ...
  const embed = new Discord.RichEmbed()
      .setColor("#000000")
      .setDescription(`
   ** :bookmark:  Help :bookmark: **
** ${prefix}shelp :pencil: لـــ ارسال الاوامر بي الخاص **

=======================

** ${prefix}chelp :envelope_with_arrow:  لـــ ارسال الاوامر بي الشات **

=======================
**الرومات المخصصه للبوت** 
** log **
** Welcome **
 `)
...
client.on("message", function(message) {
   ...
   if(message.content.startsWith(prefix + "shelp")) {
    ...
    .addField("Admin | الاداريه","👑",true)
    .addField("Games | العاب","🎮",true)
        .addField("BotRooms | الموسيقي","🤖",true)
 ...
 ***
    ...
    ❖ ${prefix}avatar server :writing_hand: لعرض افتار السيرفر
	...
	❖ ${prefix}invitelink :writing_hand: لمعرفه جميع روابط الدعوات التي دعوت بها
	❖ ${prefix}new :writing_hand: لفتح تكيت 
	❖ ${prefix}close :writing_hand: لغلق التيكت 
	❖ ${prefix}skin :writing_hand: لعرض سكنك بي ماين كرافت
	❖ ${prefix}embed :writing_hand: للارسال الكلام بي امبيد 
	❖ ${prefix}color :writing_hand: لتغير لون اسمك برقم اللون
	❖ ${prefix}link :writing_hand: للاعطاءك لينك السيرفر  
	❖ ${prefix}ping :writing_hand: لمعرفه سرعه اتصال البوت 
	***
	<!LG ÐrĀčuĿĀ#6764>
`)
 ...
 ***
    ...
    ✴ ${prefix}channels :writing_hand: لانشاء روم كتابي او صوتي او فئه 
	...
	✴ ${prefix}roleremove :writing_hand: لي سحب رتبه من شخص او للكل 
	✴ ${prefix}mute :writing_hand: لي اعطاء ميوت كتابي لشخص مع تحديد الوقت 
	✴ ${prefix}vmute :writing_hand: لي اعطاء ميوت صوتي لشخص 
	✴ ${prefix}move :writing_hand: لسحب العضو الي الروم الصوتي
    ✴ ${prefix}move all :writing_hand: لسحب كل من في الرومات الصوتيه الي رومك الصوتي
	...
	✴ ${prefix}Rainbow :writing_hand: لعمل رتبه قوس قزح وتشغلها 
	✴ ${prefix}bc :writing_hand: لارسال برود كاسيت للكل 
	✴ ${prefix}obc :writing_hand: لي ارسال برود كاسيت لـ اون لاين فقط 
	✴ ${prefix}rolebc :writing_hand: لارسال برود كاسيت لرتبه معينه 
	rolebc الرساله @<Name role > 
	✴ ${prefix}send :writing_hand: لي ارسال برود كاسيت لشخص معين 
	send @<Name> الرساله
	✴ ${prefix}antispread on :writing_hand: لتشغيل منع نشر الروابط 
	✴ ${prefix}antispread off :writing_hand: ليقاف تشغيل منع الروابط 
	✴ ${prefix}sp :writing_hand: لتحويل حاله البوت الي بلينق
	✴ ${prefix}sw :writing_hand: لتحويل حاله البوت الي واتشنق
	✴ ${prefix}sl :writing_hand: لتحويل حاله البوت الي لستينق
	✴ ${prefix}ss :writing_hand: لتحويل حاله البوت الي استريمنق
	***
	<!LG ÐrĀčuĿĀ#6764>
`)
   message.author.sendEmbed(embed)
...
***
   💠 ${prefix}لعبه صراحه :writing_hand: صراحه
   💠 ${prefix} لعبه لو خيروك  :writing_hand: خيروك
   💠 ${prefix} لعبه كت تويت :writing_hand:  كت تويت
   💠 ${prefix}لعبه اساله فورت نايت و ببجي :writing_hand: فورت نايت 
 ...
 ***
	:cd: ${prefix}play :writing_hand: لتشغيل الاغنيه بي رابط او بي اسم 
    :cd: ${prefix}stop :writing_hand: لايقاف الاغنيه المشغله حاليا 
    :cd: ${prefix}run :writing_hand لمواصله تشغيل الاغنيه بعد ايقافها 
	:cd: ${prefix}skip :writing_hand: لتجاوز الاغنيه الحاليه 
    :cd: ${prefix}restart :writing_hand: لـ اعاده تشغيل الاغنيه مره اخري 
	:cd: ${prefix}song :writing_hand: لمعرفه اسم الاغنيه المشغله حاليا 
    :cd: ${prefix}vol  :writing_hand: لتحكم بي درجه الصوت 
    :cd: ${prefix}queue :writing_hand: لعرض قائمه التشغيل 
	***
<!LG ÐrĀčuĿĀ#6764>
`)
...
client.on('message', message => {	
    if(message.content.startsWith(prefix + "toggleAntihack")) {
        ...
        if(!antihack[message.guild.id]) antihack[message.guild.id] = {
          ...
          if(antihack[message.guild.id].onoff === 'On') return [message.channel.send(`**The AntiBots Is ON!**`), antihack[message.guild.id].onoff = 'Off']
...
client.on('voiceStateUpdate', (old, now) => {
  const channel = client.channels.get('439020996171923466');
...
client.on('guildMemberRemove', member => {
    ...
    var channel =member.guild.channels.find('leves')
    if (!channel) return;
    channel.send({embed : embed});
    })

let room = "581497235431358485"
...
invites.forEach(invite => {
if (invite.inviter === message.author) {
codes.push(`discord.gg/${invite.server}`)
}
})
}).then(m => {
if (codes.length < 0) {
    var embed = new Discord.RichEmbed()
.setColor("#000000")
.addField(`Your invite server in ${message.guild.name}`, `You currently don't have any active invites! Please create an invite and start inviting, then you will be able to see your codes here!`)
message.author.send({ embed: embed });
return;
} else {
    var embed = new Discord.RichEmbed()
.setColor("#000000")
.addField(`Your invite server in ${message.guild.name}`, `Invite server:\n${codes.join("\n")}`)
message.author.send({ embed: embed });
return;
}
})
...
var args = message.content.split(" ").slice(1);
    ...
    message.channel.send(`<@${tomute.id}> ** :zipper_mouth: Was Given Muted ** : **${ms(ms(mutetime))}** `);
    setTimeout(function(){
      tomute.removeRole(muterole.id);
      message.channel.send(`<@${tomute.id}> **The Time Passed And The Person Muted Was Disassociated**:white_check_mark:`);
    }, ms(mutetime));
...
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**I Don't Have `MANAGE_ROLES` Permission**").then(msg => msg.delete(6000))
  ...
  message.channel.sendMessage("**Has been Unpacked Muted Successfully **:white_check_mark:");
...
client.on('message',msg=>{
  if(msg.content==='هلا')
msg.reply('**:heart_exclamation:  هلاوات يا حب اطلق ولكم :heart_exclamation: **');
})
client.on('message',msg=>{
  if(msg.content==='.')
msg.reply('**:rolling_eyes:  يا نقطه عمري :heart:**');
})

client.on('message' , message => {
  if(message.author.bot) return;
  if(message.content.startsWith(prefix + "ping")) {
 message.channel.send('```Is Ping Bot ```').then((msg) => {
...
function play(guild, song) {
  ...
  .setTitle(`**${fuck.title}**`)
  .setURL(fuck.url)
  .addField('Duration video ' , `${song.time}`, true)
  .addField('Name the channel ' , `${song.best}`, true)
  .addField('Making time ' , `${fuck.datePublished}`, true)
  .addField('volume ' , `${song.Volume}`, true)
  .addField('channel ' , `${song.channel}`, true)
  .addField('Requested by ' , `${song.by}`, true)
  .addField('Views ' , `${fuck.views}`, true)
  .addField('Likes👍 ' , `${fuck.likeCount}`, true)
